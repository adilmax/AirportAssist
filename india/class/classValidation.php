...
class inputValidation{
    ...
    public function checkRequiredValueWithLabel($data , $required, $label){
        $error = [];
        foreach($required as $key=>$value){
            if(isset($data[$value])){
                if($data[$value] == ''){               
    ...
    public function checkRequiredValueWithLabelArrival($data , $required, $label){
        $errorArrival = [];
        foreach($required as $key=>$value){
            if(isset($data[$value])){
                if($data[$value] == ''){               
                    $errorArrival [] = 'Please enter '.$label[$value];
                }
            }else{
                $errorArrival [] = 'Please enter '.$label[$value];
            }
     ...
     public function checkRequiredValueWithLabelDeparture($data , $required, $label){
        $errorDepar = [];
        foreach($required as $key=>$value){
            if(isset($data[$value])){
                if($data[$value] == ''){               
                    $errorDepar[] = 'Please enter '.$label[$value];
                }
            }else{
                $errorDepar[] = 'Please enter '.$label[$value];
            }
      ...
      public function checkRequiredValueWithLabelTransit($data , $required, $label){
        $errorTransit = [];
        foreach($required as $key=>$value){
            if(isset($data[$value])){
                if($data[$value] == ''){               
                    $errorTransit[] = 'Please enter '.$label[$value];
                }
            }else{
                $errorTransit[] = 'Please enter '.$label[$value];
            }
    ...
    public function checkRequiredValueWithLabelLimousine($data , $required, $label){
        $errorLimousine = [];
        foreach($required as $key=>$value){
            if(isset($data[$value])){
                if($data[$value] == ''){               
                    $errorLimousine[] = 'Please enter '.$label[$value];
                }
            }else{
                $errorLimousine[] = 'Please enter '.$label[$value];
            }
        }
        return $errorLimousine;
    }
