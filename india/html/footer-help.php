...
<button type="button" class="btn help-main-button" data-toggle="modal" data-target="#helpModal" id="help_dialogue_button" >
        <span class="fa fa-question-circle help-icon" aria-hidden="true"></span>Help
</button>
<div id="helpModal" class="modal fade" role="dialog">
  <div class="modal-dialog help-dialogue-box">

    <!-- Modal content-->
    <div class="modal-content" >
        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal">&times;</button>
            <h4 class="modal-title"><b>Leave us a message</b></h4>
        </div>
        <form method="post" action="" id="form_contact">
            <div class ="row">
                <div id="help_success_status" class="alert alert-success dispaly_none text-center" style="font-size:12px;padding-top: 10px;width:300px;margin-left:40px;margin-top:10px;"></div>
                <div id="help_failure_status" class="alert alert-danger dispaly_none text-center" style="font-size:12px;padding-top: 10px;width:300px;margin-left:40px;margin-top:10px;"></div>
                <div class="modal-body help-box">
                    <div class="form-group">
                        <label class="control-label help-font" for="name">Name</label>
                    ...
                    <div class="form-group">
                        <label class="control-label help-font" for="email" >Email</label>
                        <input type="email" class="form-control" id="help_email" name="help_email" placeholder="Enter Your Email"
                            data-validation="required email" 
                            data-validation-error-msg-required=" ">  
                    </div>
                    <div class="form-group">
                        <label class="control-label help-font" for="message">Message</label>                       
                           <textarea class="form-control select_text_color" rows="3" id="help_message" name="help_message" placeholder="Message" style="resize: vertical;"
                            data-validation="required" 
                            data-validation-error-msg-required=" "></textarea>
                    </div>
                </div>
            </div>  
      
            <div class="modal-footer">
              <button type="button" class="btn help_button" data-dismiss="modal">Close</button>
              <button type="button" class="btn disabled help_button" id="help_send" name="help_send">Send</button>
            </div>
        </form>
    </div>
    ...
    <div class="row stamp-color padding-left">
        ...
        <div class="col-lg-2 col-md-2 col-xs-4">
            ...
            <div class="col-lg-10 col-md-10 col-xs-8">
                <h2 class="xs-font white stamps-top">We Ensure Swift, Smooth And Safe Passage Through Airports</h2>
            </div>
        ...
        <div class="container"> 
            <div class="row">
                ...
                <div class="col-lg-10 col-md-10 col-sm-10 col-xs-10">
                    <a href="http://murgency.com" target="_blank"><img src="../image/footer-img.png" class="img-responsive center-block"  alt="Airport Assistance" title="MUrgency_logo"/></a>
                </div>
                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-1"></div>
           </div>
            <div class="row media-icom">
                ...
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-8 image-padding" >
                    ...
                    <a href="https://twitter.com/MUAirportAssist" class=" text-center pull-left" target="_blank"><img class="img-responsive img-rounded" alt="twitter_icon" title="Airport Assistance" src="../image/icons/TwitterDesat.png" style="width:30px;padding-right:2px;" onmouseover="hover(this,'TwitterColor.png');" onmouseout="unhover(this,'TwitterDesat.png');"></a>
            ...
            <div class="row">
                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-1"></div>
                <div class="col-lg-10 col-md-10 col-sm-10 col-xs-10">
                    <div class="text-center xs_footer copy-right">MUAirportAssist@MUrgency.com</div>
                </div>
                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-1"></div>
            </div>
            <div class="row">
                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-1"></div>
                <div class="col-lg-10 col-md-10 col-sm-10 col-xs-10">
                    <div class="text-center xs_footer copy-right">© Copyright <?=date("Y");?> MUrgency Inc.</div>
                 </div>
                <div class="col-lg-1 col-md-1 col-sm-1 col-xs-1"></div>
            </div>
        </div> 
...
$(document).ready(function () {
    $("#visaReq").click(function () {
        $("#visaReqList").animate({height: 'toggle'}, "slow");
    });
    $("#cInfo").click(function () {
        $("#cInfoList").animate({height: 'toggle'}, "slow");
    });
    $("#sightSee").click(function () {
        $("#sightSeeList").animate({height: 'toggle'}, "slow");
    });
});
...
$("#twittersharer").click(function(){
    ...
    var url = "https://twitter.com/share?url=''&hashtags=\n\
...
function googleplusbtn(url) {
  ...
  return false;
    ...
    $("#help_name").change(function(){
      var name =    $("#help_name").val();
      var email =    $("#help_email").val();
      var message =    $("#help_message").val();
      checkHelpFormValue(name,email,message);
    }); 
    
    $("#help_email").change(function(){    
        var email =    $("#help_email").val();
        var name =    $("#help_name").val();
        var message =    $("#help_message").val();
        checkHelpFormValue(name,email,message);
    }); 
    $("#help_message").change(function(){    
        var message =    $("#help_message").val();
        var email =    $("#help_email").val();
        var name =    $("#help_name").val();     
        checkHelpFormValue(name,email,message);
    ...
    function checkHelpFormValue(name,email,message){
     
        if(name !== "" &&  email !=="" && message !== ""){
            if(validateEmail(email)){
                $('#help_send').removeClass('disabled');
            }else{
                if(!$("#help_send").hasClass( "disabled" )){
                    $('#help_send').addClass('disabled'); 
                } 
            }
        }else{
            if(!$("#help_send").hasClass( "disabled" )){
    ...
    $(function () {
        $('#help_send').click( function() {  
        var name = $('#help_name').val()
        var email = $('#help_email').val()
        var message = $('#help_message').val()
        
        if((name !== "") && (email !== "") && (message !== "")){ 
            $.post("helpcontact.php",{name:name, email:email, message:message},function(data){
                
                if(data){
                   $('#help_success_status').html('Your message has been sent successfully');
                   $('#help_success_status').show();
                   $('#help_name').val("");
                   $('#help_email').val("");
                   $('#help_message').val("")
                }
                else{
                    $('#help_failure_status').html('Something went wrong.Please try after some time');
                    $('#help_failure_status').show();
                }
            });
        }else{
            $('#help-error-status').html('Please enter all the mandatory field.');
            $('#help-error-status').show();
        }
        });
...
<script>
    ...
    function getCookie(cname) {
        ...
        for (var i = 0; i < ca.length; i++) {
            var c = ca[i];
            while (c.charAt(0) == ' ') c = c.substring(2);
    ...
    jQuery(window).load(function () {
        $("#loader-wrapper, .demo").fadeOut("slow");
        var timer = window.setTimeout(show, 3000);

    });

    // Close button hide show

    var $dialog = $('#overlay');
    var $dialogCloseBtn = $dialog.parent().find('.close');
    ...
    $('#subscribe').click(function () {
        ...
        if (emailaddress != "") {
            if (!validateEmail(emailaddress)) {
                $('#error_msg').html('<p>Please enter a valid email id.</p>');
            } else {
                $('#error_msg').html('');
            ...
            $('#error_msg').html('<p>Please enter the value</p>');
        }
    });

    $('#email').click(function () {
        $('#form_Comment').fadeOut('slow');
        $('#form_Comment2').fadeOut('slow');
        $('#form_Comment3').fadeOut('slow');
    });
