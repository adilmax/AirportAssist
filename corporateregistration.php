        ...
        if($emailStatus){
            ...
            if($emailExcist){
                ...
                if($mobileStatus){
                   ...
                   $fullName = $data['contactPerson']." ".$data['lastName'];
                   $userMailStatus = $airportCorporate->sendMailToUser($data['email'],$fullName);
                   $corpoUserMailStatus = $airportCorporate->sendMailToCorporateUser($data['email'],$fullName,$signUpStatus);
                   header("Location:corporatethankyou");
                }
            }else{
